...
namespace
{
  const command_line::arg_descriptor<std::string> arg_config_file = {"config-file", "Specify configuration file", "configs/coin.conf"};
  ...
  const command_line::arg_descriptor<std::string> arg_CRYPTONOTE_NAME  = {"CRYPTONOTE_NAME", "Cryptonote name. Used for storage directory", "univercoin"};
  ...
  const command_line::arg_descriptor<uint8_t>    arg_ZAWY_DIFFICULTY_BLOCK_VERSION  = {"ZAWY_DIFFICULTY_DIFFICULTY_BLOCK_VERSION", "uint8_t", '3'};
  ...
  command_line::add_arg(desc_cmd_sett, arg_genesis_block_reward_address);
    ...
    bool r = command_line::handle_error_helper(desc_options, [&]()
    {
      ...
      else
      {
        ...
        return false;
		*/
		  std::cout << "univercoin(UNC) starting ..." << std::endl;
      }
      po::notify(vm);
...
{
  if (command_line::get_arg(vm, arg_CRYPTONOTE_NAME) == "univercoin") {
